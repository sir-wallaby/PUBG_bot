...
{
    public class pubgDuoCommand :ModuleBase<SocketCommandContext>
    {
        [Command("du")]
        public async Task duoAllRegions([Remainder] string user)
        {
            ...
            try
            {
                var SquadRank = stats.Stats.Find(x =>
                    x.Mode == Mode.Duo && x.Region == Region.AGG && x.Season == Seasons.EASeason3).Stats.Find(x =>
