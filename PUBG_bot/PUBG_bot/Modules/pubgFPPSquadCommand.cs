...
{
    public class pubgFPPSquadCommand : ModuleBase<SocketCommandContext>
    {
        [Command("sqfpp")]
        [Summary("Squad statistics for PUBG based from PUBGOnline")]
        public async Task FPPsquadAllRegions([Remainder] string user)
        {
            ...
            try
            {
                var SquadRank = stats.Stats.Find(x =>
                    x.Mode == Mode.SquadFpp && x.Region == Region.AGG && x.Season == Seasons.EASeason4).Stats.Find(x =>
